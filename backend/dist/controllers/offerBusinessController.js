...
export async function createOfferController(request, response) {
    const { title, description, category, type, oldPrice, newPrice, stock, pickupWindow, pickupLimit, allergens, imageUrl, estimatedWeightInKg, isVisible } = request.body;
...
export async function updateOfferController(request, response) {
    ...
    const { title, description, category, type, oldPrice, newPrice, stock, pickupWindow, pickupLimit, allergens, imageUrl, estimatedWeightInKg } = request.body;
...
export async function updateBusinessProfileController(request, response) {
    const { name, category, description, city, address, closingTime, logoUrl, paymentInfo } = request.body;
...
export async function getBusinessProfileController(request, response) {
    const business = await findBusinessById(request.user.businessId);
    if (!business)
        return fail(response, 404, "Comercio no encontrado.");
    response.json({ success: true, data: { business } });
}
export async function listBusinessOffersController(request, response) {
...
export async function toggleOfferVisibilityController(request, response) {
    const offerId = request.params.id;
    if (!offerId || Array.isArray(offerId))
        return fail(response, 400, "ID de oferta inválido.");
    const { isVisible } = request.body;
    if (typeof isVisible !== "boolean")
        return fail(response, 400, "isVisible debe ser true o false.");
    const updated = await updateOfferById(offerId, request.user.businessId, { isVisible });
    if (!updated)
        return fail(response, 404, "Oferta no encontrada o no pertenece a tu comercio.");
