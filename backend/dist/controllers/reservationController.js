import { createReservation, listReservationsForUser, updateReservationStatus } from "../services/reservationService.js";
export async function listReservationsController(request, response) {
    const { page, limit } = request.query;
    const reservations = await listReservationsForUser(request.user, page ? Number(page) : undefined, limit ? Number(limit) : undefined);
    response.json({ success: true, data: { reservations } });
}
export async function updateReservationStatusController(request, response) {
    ...
    const reservation = await updateReservationStatus(reservationId, status, request.user);
    if (!reservation) {
...
export async function createReservationController(request, response) {
    const { offerId, quantity } = request.body;
